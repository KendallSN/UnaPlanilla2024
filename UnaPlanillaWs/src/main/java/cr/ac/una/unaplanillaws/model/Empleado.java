...
@NamedQueries({
    ...
    @NamedQuery(name = "Empleado.findByCedulaNombreApellidos", query = "SELECT e FROM Empleado e WHERE UPPER(e.nombre) like :nombre and UPPER(e.cedula) like :cedula and UPPER(e.primerApellido) like :pApellido and UPPER(e.segundoApellido) like :sApellido", hints = @QueryHint(name = "eclipselink.refresh", value = "true")),    @NamedQuery(name = "Empleado.findByUsuClave", query = "SELECT e FROM Empleado e WHERE e.usuario = :usuario and e.clave = :clave", hints = @QueryHint(name = "eclipselink.refresh", value = "true"))
})
